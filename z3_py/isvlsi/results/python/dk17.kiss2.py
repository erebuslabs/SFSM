...
for bits in range(int(ceil(log(8)/log(2))+4),  int(ceil(log(8)/log(2))-1), -1):
	st_s10000000, st_s01000000, st_s00100000, st_s00010000, st_s00001000, st_s00000100, st_s00000010, st_s00000001 = BitVecs('st_s10000000 st_s01000000 st_s00100000 st_s00010000 st_s00001000 st_s00000100 st_s00000010 st_s00000001',bits)

	s = Solver();
	s.set("timeout", 30000)
	s.add(Distinct(st_s10000000,st_s01000000,st_s00100000,st_s00010000,st_s00001000,st_s00000100,st_s00000010,st_s00000001))
	...
	s.add(
	Sum([ ZeroExt(int(ceil(log(bits)/log(2))+1), Extract(i,i,(st_s10000000 ^ st_s10000000))) for i in range(bits) ]) ==
	...
	s.add(
	Sum([ ZeroExt(int(ceil(log(bits)/log(2))+1), Extract(i,i,(st_s00010000 ^ st_s00010000))) for i in range(bits) ]) ==
	Sum([ ZeroExt(int(ceil(log(bits)/log(2))+1), Extract(i,i,(st_s01000000 ^ st_s00000100))) for i in range(bits) ])
	)
	s.add(
