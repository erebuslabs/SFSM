...
for bits in range(int(ceil(log(4)/log(2))+4),  int(ceil(log(4)/log(2))-1), -1):
	st_state1, st_state2, st_state3, st_state4 = BitVecs('st_state1 st_state2 st_state3 st_state4',bits)

	s = Solver();
	s.set("timeout", 30000)
	s.add(Distinct(st_state1,st_state2,st_state3,st_state4))
	...
	s.add(
	Sum([ ZeroExt(int(ceil(log(bits)/log(2))+1), Extract(i,i,(st_state1 ^ st_state1))) for i in range(bits) ]) ==
	...
	s.add(
	Sum([ ZeroExt(int(ceil(log(bits)/log(2))+1), Extract(i,i,(st_state2 ^ st_state2))) for i in range(bits) ]) ==
	Sum([ ZeroExt(int(ceil(log(bits)/log(2))+1), Extract(i,i,(st_state3 ^ st_state4))) for i in range(bits) ])
	)
	s.add(
	...
	s.add(
	Sum([ ZeroExt(int(ceil(log(bits)/log(2))+1), Extract(i,i,(st_state3 ^ st_state3))) for i in range(bits) ]) ==
	Sum([ ZeroExt(int(ceil(log(bits)/log(2))+1), Extract(i,i,(st_state3 ^ st_state4))) for i in range(bits) ])
	)
	s.add(
	Sum([ ZeroExt(int(ceil(log(bits)/log(2))+1), Extract(i,i,st_state4)) for i in range(bits) ]) ==
