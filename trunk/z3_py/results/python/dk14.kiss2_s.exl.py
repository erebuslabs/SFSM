...
for bits in range(int(ceil(log(10)/log(2))+4),  int(ceil(log(10)/log(2))-1), -1):
	st_state_1, st_state_3, st_state_2, st_state_3_n, st_state_4, st_state_5, st_state_6, st_state_7, st_state_2_n, st_state_4_n = BitVecs('st_state_1 st_state_3 st_state_2 st_state_3_n st_state_4 st_state_5 st_state_6 st_state_7 st_state_2_n st_state_4_n',bits)

	s = Solver();
	s.set("timeout", 30000)
	s.add(Distinct(st_state_1,st_state_3,st_state_2,st_state_3_n,st_state_4,st_state_5,st_state_6,st_state_7,st_state_2_n,st_state_4_n))
	...
	s.add(
	Sum([ ZeroExt(int(ceil(log(bits)/log(2))+1), Extract(i,i,st_state_3)) for i in range(bits) ]) ==
	Sum([ ZeroExt(int(ceil(log(bits)/log(2))+1), Extract(i,i,st_state_3_n)) for i in range(bits) ])
	)
	s.add(
	Sum([ ZeroExt(int(ceil(log(bits)/log(2))+1), Extract(i,i,(st_state_3 ^ st_state_3_n))) for i in range(bits) ]) ==
	Sum([ ZeroExt(int(ceil(log(bits)/log(2))+1), Extract(i,i,(st_state_4 ^ st_state_7))) for i in range(bits) ])
	)
	s.add(
	Sum([ ZeroExt(int(ceil(log(bits)/log(2))+1), Extract(i,i,st_state_3_n)) for i in range(bits) ]) ==
	...
	s.add(
	Sum([ ZeroExt(int(ceil(log(bits)/log(2))+1), Extract(i,i,(st_state_3_n ^ st_state_3))) for i in range(bits) ]) ==
	Sum([ ZeroExt(int(ceil(log(bits)/log(2))+1), Extract(i,i,(st_state_4 ^ st_state_7))) for i in range(bits) ])
	)
	s.add(
	...
	s.add(
	Sum([ ZeroExt(int(ceil(log(bits)/log(2))+1), Extract(i,i,(st_state_2 ^ st_state_2_n))) for i in range(bits) ]) ==
	Sum([ ZeroExt(int(ceil(log(bits)/log(2))+1), Extract(i,i,(st_state_4 ^ st_state_7))) for i in range(bits) ])
	)
	s.add(
	Sum([ ZeroExt(int(ceil(log(bits)/log(2))+1), Extract(i,i,st_state_2_n)) for i in range(bits) ]) ==
	...
	s.add(
	Sum([ ZeroExt(int(ceil(log(bits)/log(2))+1), Extract(i,i,(st_state_2_n ^ st_state_2))) for i in range(bits) ]) ==
	Sum([ ZeroExt(int(ceil(log(bits)/log(2))+1), Extract(i,i,(st_state_4 ^ st_state_7))) for i in range(bits) ])
	)
	s.add(
	Sum([ ZeroExt(int(ceil(log(bits)/log(2))+1), Extract(i,i,st_state_5)) for i in range(bits) ]) ==
	...
	s.add(
	Sum([ ZeroExt(int(ceil(log(bits)/log(2))+1), Extract(i,i,(st_state_4 ^ st_state_4_n))) for i in range(bits) ]) ==
	Sum([ ZeroExt(int(ceil(log(bits)/log(2))+1), Extract(i,i,(st_state_4 ^ st_state_7))) for i in range(bits) ])
	)
	s.add(
	Sum([ ZeroExt(int(ceil(log(bits)/log(2))+1), Extract(i,i,st_state_4_n)) for i in range(bits) ]) ==
	...
	s.add(
	Sum([ ZeroExt(int(ceil(log(bits)/log(2))+1), Extract(i,i,(st_state_4_n ^ st_state_4))) for i in range(bits) ]) ==
	Sum([ ZeroExt(int(ceil(log(bits)/log(2))+1), Extract(i,i,(st_state_4 ^ st_state_7))) for i in range(bits) ])
	)
	s.add(
	Sum([ ZeroExt(int(ceil(log(bits)/log(2))+1), Extract(i,i,st_state_7)) for i in range(bits) ]) ==
