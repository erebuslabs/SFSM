...
for bits in range(int(ceil(log(17)/log(2))+4),  int(ceil(log(17)/log(2))-1), -1):
	st_*, st_init0, st_init0_n, st_init1, st_init1_n, st_init2, st_init4, st_init4_n, st_IOwait, st_IOwait_n, st_read0, st_write0, st_RMACK, st_WMACK, st_RMACK_n, st_WMACK_n, st_read1 = BitVecs('st_* st_init0 st_init0_n st_init1 st_init1_n st_init2 st_init4 st_init4_n st_IOwait st_IOwait_n st_read0 st_write0 st_RMACK st_WMACK st_RMACK_n st_WMACK_n st_read1',bits)

	s = Solver();
	s.set("timeout", 30000)
	s.add(Distinct(st_*,st_init0,st_init0_n,st_init1,st_init1_n,st_init2,st_init4,st_init4_n,st_IOwait,st_IOwait_n,st_read0,st_write0,st_RMACK,st_WMACK,st_RMACK_n,st_WMACK_n,st_read1))
	...
	s.add(
	Sum([ ZeroExt(int(ceil(log(bits)/log(2))+1), Extract(i,i,(st_* ^ st_init0))) for i in range(bits) ]) ==
