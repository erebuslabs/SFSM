...
for bits in range(int(ceil(log(10)/log(2))+4),  int(ceil(log(10)/log(2))-1), -1):
	st_1, st_7, st_0, st_2, st_5, st_3, st_8, st_4, st_6, st_9 = BitVecs('st_1 st_7 st_0 st_2 st_5 st_3 st_8 st_4 st_6 st_9',bits)

	s = Solver();
	s.set("timeout", 30000)
	s.add(Distinct(st_1,st_7,st_0,st_2,st_5,st_3,st_8,st_4,st_6,st_9))
	...
	s.add(
	Sum([ ZeroExt(int(ceil(log(bits)/log(2))+1), Extract(i,i,(st_9 ^ st_6))) for i in range(bits) ]) ==
	Sum([ ZeroExt(int(ceil(log(bits)/log(2))+1), Extract(i,i,(st_9 ^ st_0))) for i in range(bits) ])
	)
	s.add(
	Sum([ ZeroExt(int(ceil(log(bits)/log(2))+1), Extract(i,i,st_9)) for i in range(bits) ]) ==
